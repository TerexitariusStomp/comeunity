...
export async function onRequestGet({ request, env }) {
  ...
  const total = await env.DB.prepare('SELECT COUNT(*) as c FROM organizations').first();
  const totalJobs = await env.DB.prepare('SELECT COUNT(*) as c FROM volunteer_opportunities').first();

  const sources = await env.DB.prepare('SELECT source, COUNT(*) as c FROM organizations GROUP BY source').all();
  const countries = await env.DB.prepare('SELECT country, COUNT(*) as c FROM organizations GROUP BY country').all();

  const volunteers = await env.DB.prepare('SELECT COUNT(*) as c FROM organizations WHERE accepts_volunteers = 1').first();
  const visitors = await env.DB.prepare('SELECT COUNT(*) as c FROM organizations WHERE accepts_visitors = 1').first();
  const shortterm = await env.DB.prepare('SELECT COUNT(*) as c FROM organizations WHERE accepts_shortterm = 1').first();
  const longterm = await env.DB.prepare('SELECT COUNT(*) as c FROM organizations WHERE accepts_longterm = 1').first();
  const jobs = await env.DB.prepare('SELECT COUNT(*) as c FROM organizations WHERE has_jobs = 1').first();
  const stays = await env.DB.prepare('SELECT COUNT(*) as c FROM organizations WHERE has_stays = 1').first();

  return jsonResponse({
    total_organizations: total?.c || 0,
    total_opportunities: totalJobs?.c || 0,
    by_source: Object.fromEntries((sources.results || []).map(r => [r.source, r.c])),
    by_country: Object.fromEntries((countries.results || []).map(r => [r.country, r.c])),
    feature_counts: {
      accepts_volunteers: volunteers?.c || 0,
      accepts_visitors: visitors?.c || 0,
      accepts_shortterm: shortterm?.c || 0,
      accepts_longterm: longterm?.c || 0,
      has_jobs: jobs?.c || 0,
      has_stays: stays?.c || 0,
    }
  });
