...
export async function onRequestGet({ request, env }) {
  ...
  let sql = `SELECT s.*, o.name as org_name, o.website as org_website, o.country as org_country, o.city as org_city, o.latitude as org_lat, o.longitude as org_lon
    FROM stays s JOIN organizations o ON s.organization_id = o.id`;
  const binds = [];
  let where = '';

  if (search) {
    where = ' WHERE s.title LIKE ? OR s.description LIKE ? OR s.stay_type LIKE ? OR o.name LIKE ? OR o.country LIKE ?';
    const s = `%${search}%`;
  ...
  const results = (rows.results || []).map(r => ({
    ...
    booking_analysis: r.booking_analysis,
