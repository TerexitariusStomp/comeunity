...
export async function onRequestGet({ request, env }) {
  ...
  let sql = `SELECT vo.*, o.name as org_name, o.website as org_website, o.country as org_country, o.city as org_city, o.latitude as org_lat, o.longitude as org_lon
    FROM volunteer_opportunities vo JOIN organizations o ON vo.organization_id = o.id`;
  const binds = [];
  let where = '';

  if (search) {
    where = ' WHERE vo.title LIKE ? OR vo.description LIKE ? OR vo.role LIKE ? OR o.name LIKE ? OR o.country LIKE ?';
    const s = `%${search}%`;
